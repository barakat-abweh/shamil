...
if(htmlspecialchars($_SERVER['REQUEST_METHOD'])=="POST"){
    if(isset($_POST['lastinterestid'])){
        ...
        if($id=="0"){
         $query="SELECT `interest_id`, `interested_user_id`,`Date`, `property_id` FROM `interested` WHERE `property_owner_id`='$userid' AND interest_id >= $id ORDER BY `Date` DESC";     
         ...
         $query="SELECT `interest_id`, `interested_user_id`,`Date`, `property_id` FROM `interested` WHERE `property_owner_id`='$userid' AND interest_id >= $id AND `seen` = 0 ORDER BY `Date` DESC";     
        $Result=$database->query($query);
                while($res=$database->fetchArray($Result)){
               $user->setId($res['interested_user_id']);
               $property->setId($res['property_id']);
